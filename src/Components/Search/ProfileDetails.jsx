...
const ProfileDetails = () => {
    ...
    if (currentStep === 2) {
      ...
      if (formData.tipoAjuda === "Alimentos" && userType === "Doador") {
        if (formData.quantidadeProdutos === "Personalizado") {
          const totalQuantidade = formData.alimentosDetails.reduce(
            (sum, alimento) => {
          ...
          if (totalQuantidade > parseInt(formData.quantidadePersonalizada)) {
            setError(
              "A soma das quantidades dos alimentos não pode ser maior que a quantidade personalizada."
            );
            return;
          }
        }
  ...
  const handleChange = (e) => {
    ...
    const newValue =
      name === "quantidadePersonalizada" || name === "quantidade"
        ? value === ""
          ? ""
          : parseInt(value) // Garante que o valor seja um número ou vazio
        : value;
      ...
      if (name === "tipoAjuda") {
        return {
          ...
          alimentosDetails:
            value === "Alimentos"
              ? [{ tipo: "", quantidade: "" }]
              : prevData.alimentosDetails,
          higieneDetails:
            value === "Higiene"
              ? [{ produto: "", marca: "" }]
              : prevData.higieneDetails,
          voluntariaDetails:
            value === "Ajuda voluntária"
              ? [{ horas: "", habilidades: "" }]
              : prevData.voluntariaDetails,
        };
      }
  ...
  const handleDynamicFieldChange = (e, type, index) => {
    ...
    const newValue =
      name === "quantidade" || name === "quantidadePersonalizada"
        ? value === ""
          ? ""
          : parseInt(value)
        : value;
